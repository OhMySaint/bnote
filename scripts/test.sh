...
CORE=(BNote/Models/PageConfig.swift BNote/Models/Note.swift BNote/Models/Tag.swift
      BNote/Views/PageHeaderView.swift BNote/Views/CoverStyle.swift BNote/Views/TagViews.swift
      BNote/Editor/*.swift)
UI=(BNote/Models/*.swift BNote/Editor/*.swift
...
run() {
  ...
  "$OUT/$name" 2>&1 | grep -vE "^$|CoreData: debug" | tail -n +1
}
